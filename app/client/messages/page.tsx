...
import { conversations, getMessagesByConversation } from "@/lib/client/data"
...
export default function ClientMessagesPage() {
  ...
  return (
    <div className="pb-20">
      <PageHeader eyebrow="Client Portal" title="Messages" description="Chat with your Triodev team in real-time." />
        ...
        <Card className="border-primary/16 bg-surface lg:col-span-2 flex flex-col overflow-hidden">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={cn("flex", msg.senderId === "client" ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  msg.senderId === "client" ? "bg-primary text-background rounded-tr-sm" : "bg-secondary border border-primary/10 text-foreground rounded-tl-sm",
                  msg.pinned && "ring-2 ring-primary/30"
                )}>
                  {msg.pinned && <div className="flex items-center gap-1 text-[10px] font-mono uppercase mb-1 opacity-70"><Pin className="w-3 h-3" /> Pinned</div>}
                  {msg.type === "code" ? <pre className="text-xs font-mono overflow-x-auto whitespace-pre-wrap">{msg.content}</pre> : <p>{msg.content}</p>}
          ...
          <div className="p-3 border-t border-primary/10">
            ...
            <div className="relative">
              <Input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type a message..." className="pr-12" />
